...
版本号来自 `clipstash-next/src-tauri/tauri.conf.json` 和 `clipstash-next/package.json`，当前为 `2.1.18`。

- 主程序：`clipstash-next/src-tauri/target/release/clipstash-next.exe`
- MSI：`clipstash-next/src-tauri/target/release/bundle/msi/ClipStash Next_2.1.18_x64_en-US.msi`
- NSIS：`clipstash-next/src-tauri/target/release/bundle/nsis/ClipStash Next_2.1.18_x64-setup.exe`
- Android release APK：Gradle 输出 `clipstash-next/src-tauri/gen/android/app/build/outputs/apk/universal/release/app-universal-release.apk`，发布时复制改名为 `ClipStash.Next_<version>_android-universal-release-signed.apk`（当前为 `ClipStash.Next_2.1.18_android-universal-release-signed.apk`）。
...
2. 运行 NSIS 安装包：`clipstash-next/src-tauri/target/release/bundle/nsis/ClipStash Next_2.1.18_x64-setup.exe`。
3. 启动 ClipStash Next。
