...
版本号来自 `clipstash-next/src-tauri/tauri.conf.json` 和 `clipstash-next/package.json`，当前为 `2.1.17`。

- 主程序：`clipstash-next/src-tauri/target/release/clipstash-next.exe`
- MSI：`clipstash-next/src-tauri/target/release/bundle/msi/ClipStash Next_2.1.17_x64_en-US.msi`
- NSIS：`clipstash-next/src-tauri/target/release/bundle/nsis/ClipStash Next_2.1.17_x64-setup.exe`
- Android release APK：Gradle 输出 `clipstash-next/src-tauri/gen/android/app/build/outputs/apk/universal/release/app-universal-release.apk`，发布时复制改名为 `ClipStash.Next_<version>_android-universal-release-signed.apk`（当前为 `ClipStash.Next_2.1.17_android-universal-release-signed.apk`）。

GitHub Release 上传 Windows 用户优先使用 NSIS 安装包，同时保留 MSI 作为备用安装入口；Android 版上传 release 构建签名通用 APK，用于侧载验收。

- 安卓打包前必须确认 `src/App.tsx` 的 `CURRENT_VERSION` 与 `src-tauri/gen/android/app/tauri.properties`（gitignored，由 `tauri android build` 重新生成）已同步到目标版本。
...
2. 运行 NSIS 安装包：`clipstash-next/src-tauri/target/release/bundle/nsis/ClipStash Next_2.1.17_x64-setup.exe`。
3. 启动 ClipStash Next。
