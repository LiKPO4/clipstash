...
版本号来自 `clipstash-next/src-tauri/tauri.conf.json` 和 `clipstash-next/package.json`，当前为 `2.0.8`。

- 主程序：`clipstash-next/src-tauri/target/release/clipstash-next.exe`
- MSI：`clipstash-next/src-tauri/target/release/bundle/msi/ClipStash Next_2.0.8_x64_en-US.msi`
- NSIS：`clipstash-next/src-tauri/target/release/bundle/nsis/ClipStash Next_2.0.8_x64-setup.exe`

GitHub Release 上传 Windows 用户优先使用 NSIS 安装包，同时保留 MSI 作为备用安装入口。
...
2. 运行 NSIS 安装包：`clipstash-next/src-tauri/target/release/bundle/nsis/ClipStash Next_2.0.8_x64-setup.exe`。
3. 启动 ClipStash Next。
