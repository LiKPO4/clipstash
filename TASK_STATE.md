...
- 当前阶段推进到阶段 3 基础归档/恢复：后端 command 已有，正在接入前端受控操作并保持旧库兼容。
...
- 阶段 3 尚未对归档/恢复 UI 或手动验收入口执行真实旧库写入验收；当前 mock 测试不写真实旧库。
...
- 进入阶段 3 下一步：运行 Rust 验证命令后，对归档/恢复执行一次可回滚的真实旧库验收，并用旧 Python/Rust 只读审计核对。
