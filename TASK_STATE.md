...
- 本轮修复 Android 检查更新永久停在“检查中”：原生端缓存最后一次检查结果，前端除事件监听外增加主动轮询领取，解决 WebView 偶发漏事件；同时增加 20 秒总超时（初稿写 40 秒，实现时收紧为 20 秒），网络或原生线程异常时会恢复按钮并显示错误，不再无限等待。
...
- 已新增桌面端与 Android 端共用的“消息双击”三档设置：`编辑 / 新建 / 无效果`，默认值为“编辑”；双击消息文字或图片时会取消单击复制并执行所选动作，避免双击先复制再编辑/新建。
- 已为消息双击设置补跑验证：`npm test -- --run` 通过（`68 passed | 9 skipped`），`npm run build` 通过，`cargo fmt -- --check` 通过，`cargo test` 通过（`33 passed | 20 ignored`）。
- 已准备 `2.1.5` 双击行为验收包：设置页新增“消息双击”三档开关（`编辑 / 新建 / 无效果`，默认 `编辑`），桌面端与 Android 端消息卡片双击文字或图片都会共用同一逻辑；为避免副作用，双击时会取消单击复制，不会先复制再进入编辑/新建。发布前验证已通过：`npm test -- --run` 通过 `68 passed | 9 skipped`；`npm run build` 通过；`cargo fmt -- --check` 通过；`cargo test` 通过 `33 passed | 20 ignored`；`npm run verify:legacy-readonly` 通过，真实旧库只读审计为 `normal=5 archived=112 total=117 joined_images=130 orphan_images=0`；`npm run tauri build` 通过并生成 `ClipStash Next_2.1.5_x64_en-US.msi` 与 `ClipStash Next_2.1.5_x64-setup.exe`；`npm run tauri -- android build --apk` 通过，签名后 `ClipStash.Next_2.1.5_android-universal-release-signed.apk` 通过 `apksigner verify`，`v2=true`、`v3=true`，`aapt dump badging` 确认 `versionName='2.1.5'`、`versionCode='2001005'`。注意：本次 Android 预览 APK 因旧 preview keystore 口令不可恢复，改用新的本地预览签名证书 `clipstash-next-android-preview-2.1.5.jks`；已安装旧测试 APK 的手机可能需要先卸载再安装。
- 已准备 `2.1.6` 更新下载/测试稳定性修复验收包：Windows “下载更新”不再给 NSIS 安装包附带 `/UPDATE` 特殊参数，避免点击后直接触发当前应用退出流程；同时图片悬浮预览窗口关闭逻辑改为兼容同步值或 Promise，修复 Vitest 中 `closeHoverPreviewWindow()` 的未处理异常。发布前验证已通过：`npm test -- --run` 通过 `68 passed | 9 skipped`；`npm run build` 通过；`cargo fmt -- --check` 通过；`cargo test` 通过 `33 passed | 20 ignored`；`npm run verify:legacy-readonly` 通过，真实旧库只读审计为 `normal=5 archived=112 total=117 joined_images=130 orphan_images=0`；`npm run tauri build` 通过并生成 `ClipStash Next_2.1.6_x64_en-US.msi` 与 `ClipStash Next_2.1.6_x64-setup.exe`；`npm run tauri -- android build --apk` 通过，签名后 `ClipStash.Next_2.1.6_android-universal-release-signed.apk` 通过 `apksigner verify`，`v2=true`、`v3=true`，`aapt dump badging` 确认 `versionName='2.1.6'`、`versionCode='2001006'`。注意：本次 Android 预览 APK 使用新的本地预览签名证书 `clipstash-next-android-preview-2.1.6.jks`；已安装旧测试 APK 的手机可能需要先卸载再安装。
...
- 已准备 `2.1.3` Android 首启闪退/状态栏修复验收包：非 Windows 启动时通过 Tauri `app.path().app_data_dir()` 注入应用数据目录，避免 Android 缺少 Windows `APPDATA/LOCALAPPDATA` 时 `ensure_app_data_ready()` 失败退出；非 Windows 首次启动找不到旧库路径时会写入空迁移状态并跳过旧库迁移；Android 主界面增加 `viewport-fit=cover` 和 `safe-area-inset-top` 顶部安全区，shell 改为全宽。发布前验证已通过：`npm test -- --run` 通过 `62 passed | 9 skipped`；`npm run build` 通过；`cargo fmt -- --check` 通过；`cargo test` 通过 `33 passed | 20 ignored`；`npm run verify:legacy-readonly` 通过，真实旧库只读审计为 `normal=5 archived=112 total=117 joined_images=130 orphan_images=0`；`npm run tauri build` 通过并生成 `ClipStash Next_2.1.3_x64_en-US.msi` 与 `ClipStash Next_2.1.3_x64-setup.exe`；`npm run tauri -- android build --apk` 通过，签名后 `ClipStash.Next_2.1.3_android-universal-release-signed.apk` 通过 `apksigner verify`，`v2=true`、`v3=true`，版本号 `2.1.3`。
- 已按手机端反馈调整新建/编辑统一弹窗的 Android 形态：Android 点击待选图片不再创建独立悬停预览窗口，改为主 WebView 内预览层，避免新建消息加图后点预览白屏；Android 下选择图片、保存移动到弹窗顶部，去除右上角 X 和关闭按钮，保留点击旁边空白区关闭，并在关闭弹窗时清理预览状态。已验证 `npm test -- --run tests/App.android.test.tsx` 通过 `1 passed`；`npm test -- --run` 通过 `62 passed | 9 skipped`；`npm run build` 通过。
- 已继续收敛 Android 设置页和导出行为：Android 设置页隐藏悬浮预览延迟、滚动速度、粘贴间隔等桌面/鼠标/外部导入相关选项；Android 顶部导出按钮在 Web Share 文件分享不可用时改用 Tauri opener 打开导出的 zip 路径，并在主界面底部显示导出结果或错误，避免点击后无反馈。已验证 `npm test -- --run tests/App.android.test.tsx` 通过 `2 passed`；`npm test -- --run` 通过 `63 passed | 9 skipped`；`npm run build` 通过。
- 已将 Android 新建/编辑统一弹窗的消息输入框默认字号调整为 `1.5em`，只影响手机端 composer，桌面端保持原字号。已验证 `npm test -- --run tests/App.android.test.tsx` 通过 `2 passed`；`npm test -- --run` 通过 `63 passed | 9 skipped`；`npm run build` 通过。
- 已准备 `2.1.4` Android 体验/检查更新修复验收包：Android 新建/编辑弹窗图片点击走主 WebView 预览层，顶部只保留“图片/保存”，输入框默认 `1.5em`；Android 设置页隐藏鼠标/桌面相关选项；Android 导出按钮增加结果/错误反馈并在 Web Share 文件分享不可用时用 opener 打开 zip；检查更新改为 Tauri 后端 `fetch_latest_github_release` 用 reqwest + `User-Agent` 请求 GitHub API，避免 Android WebView 直接 fetch 可能返回 403。发布前验证已通过：`npm test -- --run` 通过 `63 passed | 9 skipped`；`npm run build` 通过；`cargo fmt -- --check` 通过；`cargo test` 通过 `33 passed | 20 ignored`；`npm run verify:legacy-readonly` 通过，真实旧库只读审计为 `normal=5 archived=112 total=117 joined_images=130 orphan_images=0`；带 `User-Agent` 的 GitHub latest 请求返回 `v2.1.3`；`npm run tauri build` 通过并生成 `ClipStash Next_2.1.4_x64_en-US.msi` 与 `ClipStash Next_2.1.4_x64-setup.exe`；`npm run tauri -- android build --apk` 通过，签名后 `ClipStash.Next_2.1.4_android-universal-release-signed.apk` 通过 `apksigner verify`，`v2=true`、`v3=true`。注意：本次因未能恢复旧预览 keystore 口令，Android APK 使用新的本地预览签名证书，已安装旧测试包的手机可能需要先卸载再安装。
- 已准备 `2.1.7` 修复验收包：桌面检查更新优先走 WebView `fetch`，避免后端阻塞网络路径导致闪退；图片悬停预览时主窗口增加压暗层；Android 点击图片复制改为提示“移动端暂不支持复制图片”并不再调用系统剪贴板；Android 导出优先通过 `MainActivity` 原生 `ACTION_SEND` + `FileProvider` 分享 zip，避免 Web Share 文件能力不稳定和 `openPath` 打不开私有目录；Android 系统返回键在编辑/新建/设置/删除/预览弹层打开时先关闭弹层，不直接退出应用。已验证：`npm test -- --run` 通过 `71 passed | 9 skipped`；`npm run build` 通过；`cargo fmt -- --check` 通过；`cargo test` 通过 `33 passed | 20 ignored`；`npm run verify:legacy-readonly` 通过，真实旧库只读审计为 `normal=5 archived=112 total=117 joined_images=130 orphan_images=0`；`npm run tauri -- android build --apk` 通过并生成 release unsigned APK；签名后 `ClipStash.Next_2.1.7_android-universal-release-signed.apk` 通过 `apksigner verify`，`v2=true`、`v3=true`，版本号 `2.1.7`、versionCode `2001007`；`npm run tauri build` 通过并生成 `ClipStash Next_2.1.7_x64_en-US.msi` 与 `ClipStash Next_2.1.7_x64-setup.exe`。
